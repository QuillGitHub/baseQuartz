package com.example.basequartz.scheduler;

import com.example.basequartz.job.HelloJob;
import org.quartz.*;
...
public class HelloScheduler {
    public static void main(String[] args) throws SchedulerException {
        ...
        JobDetail jobDeatil = JobBuilder.newJob(HelloJob.class)
                .withIdentity("myjob", "jobgroup1")// 定义标识符,（jobgroup1组名）
                .build();// 定义标识符

        // 创建一个 Trigger 实例，定义该 job 立即执行，并且每隔两秒重复执行一次，直到永远
        Trigger trigger = TriggerBuilder.newTrigger()
                .withIdentity("myTrigger","trigroup1")// 定义标识符（即使与上Jobdetail定义同一个组名，也不是一个组，因为两个类）
                .startNow()// 定义立即执行
